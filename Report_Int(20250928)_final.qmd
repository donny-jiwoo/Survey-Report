...
ggplot(summary_q17, aes(x = 문항17, y = n, fill = 문항17)) +
  ...
  geom_text(aes(label = paste0(n,"명\n(",pct,"%)"),
                y = n),  # 막대 바로 위에 위치
            hjust=0.5, vjust=-0.05, size=6, fontface="bold", lineheight=0.5) +  # 줄간격 좁게, vjust로 미세조정
...
ggplot(
  ...
  theme(
    plot.title = element_text(face = "bold", size = 32, hjust = 0.5),
    legend.position = "none",
    axis.text.x = element_text(angle = 0, vjust = 0.8, lineheight = 0.5, size = 17, margin = margin(t = 18)),  # 중복 제거 및 통합
    axis.text.y = element_text(size = 15),
...
ggplot(summary_q36, aes(x = 문항36, y = n, fill = 문항36)) +
  ...
  geom_text(aes(label = paste0(n,"명\n(",pct,"%)"),
                y = n),
            hjust=0.3, vjust=-0.2, size= 6, fontface="bold", lineheight=0.5) +
  facet_wrap(~학교급, ncol = 2, nrow = 3) +
  labs(
    title = "컨설팅 필요성 인식",
    x = "컨설팅 필요성",
...
ggplot(
  ...
  geom_text(aes(label = paste0(n,"건\n(",pct,"%)"), 
                y = n + max(n) * 0.05),
            vjust = -0.2, hjust = 0.5, size = 6, lineheight = 0.5) +
  facet_wrap(~학교급, ncol = 1, scales = "free_y") +
  labs(
    title = "컨설팅이 필요한 분야",
    x = "컨설팅 분야",
...
p_multi <- freq_multi %>%
  ...
  geom_text(aes(label = paste0(n, " (", pct, "%)")), 
            hjust = 0.5, vjust = -0.5, size = 6, fontface = "bold") +  # hjust=0.5 (중앙), vjust=-0.5 (막대 위)
  coord_flip() +
